...
class ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        //Если написать код создания метки до вызова super.loadView(), приложение экстренно завершит работу уже в процессе загрузки. Это произойдет по причине того, что корневой view сцены еще не загружен, и обращение к свойству self.view приводит к ошибке, так как до момента вызова super.loadView() свойство self.view соответствует nil, т.е. в нем нет значения.
        
    }
    
    
    override func viewDidLoad() {
        //метод viewDidLoad срабатывает когда все View из которых состоит сцена уже загружены и готовы к выводу на экран
    ...
    override func viewWillAppear(_ animated: Bool) {
        //вызывается перед тем, как графические элементы сцены будут добавлены в иерархию графических элементов
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        //В данном методе вы можете произвести действия, которые должны быть выполнены уже после отображения элементов на экране (например, запустить анимацию на сцене или синхронизировать данные с сервером)
